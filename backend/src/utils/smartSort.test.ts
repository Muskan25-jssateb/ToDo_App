import { calculateUrgencyScore, sortTasksBySmartMix } from './smartSort';
...
export const runSmartSortVerification = () => {
  ...
  const sortedList = sortTasksBySmartMix(rawList);

  console.log('\nResults after Smart Mix Sort:');
  sortedList.forEach((task, index) => {
    const score = calculateUrgencyScore(task, now);
    console.log(
      `${index + 1}. [Score: ${score.toFixed(1)}] ${task.title} (Priority: ${task.priority}, Status: ${task.status})`
    );
  });
