...
import { Task, ITask, PriorityLevel, TaskStatus } from '../models/Task';
import { AuthenticatedRequest } from '../middlewares/authMiddleware';
import { sortTasksBySmartMix } from '../utils/smartSort';
import { isInMemoryMode } from '../config/db';
import { inMemoryTasks } from '../models/inMemoryStore';
...
export const getTasks = async (
  ...
  try {
    ...
    if (isInMemoryMode()) {
      tasks = inMemoryTasks.filter((t) => t.userId.toString() === userId.toString());
      ...
      // Sort
      if (sortBy === 'smart') {
        tasks = sortTasksBySmartMix(tasks);
        ...
        const rank: Record<PriorityLevel, number> = { URGENT: 4, HIGH: 3, MEDIUM: 2, LOW: 1 };
        tasks.sort((a, b) => {
          if (a.status !== b.status) return a.status === 'PENDING' ? -1 : 1;
      ...
      const allUserTasks = inMemoryTasks.filter((t) => t.userId.toString() === userId.toString());
      const now = new Date();
      const stats = {
...
export const updateTask = async (
  ...
  try {
    ...
    if (isInMemoryMode()) {
      const task = inMemoryTasks.find((t) => t._id.toString() === id && t.userId.toString() === userId);
      if (!task) {
      ...
      if (taskDateTime !== undefined) task.taskDateTime = taskDateTime ? new Date(taskDateTime) : undefined;
      ...
      if (subtasks !== undefined) task.subtasks = Array.isArray(subtasks) ? subtasks : task.subtasks;
...
export const deleteTask = async (
  ...
  try {
    ...
    if (isInMemoryMode()) {
      const idx = inMemoryTasks.findIndex((t) => t._id.toString() === id && t.userId.toString() === userId);
      if (idx === -1) {
        res.status(404).json({ success: false, message: 'Task not found or unauthorized.' });
        return;
      }
